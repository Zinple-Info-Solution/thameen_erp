...
# Accounts that represent freight income rather than goods income.
def _transport_income_account():
	return frappe.db.get_single_value("Thameen Fleet Settings", "transportation_income_account")
...
def get_gl_totals(filters, cost_centers):
	...
	for row in rows:
		bucket = out.setdefault(
			row.cost_center, {"revenue": 0, "direct_cost": 0, "periodic_cost": 0}
		)
		if row.root_type == "Income":
			bucket["revenue"] += flt(row.credit) - flt(row.debit)
