...
def execute(filters=None):
	...
	trips = frappe.db.sql(
		...
		select dt.name as trip, dt.status, dt.departure_time, dt.vehicle, dt.driver,
		       dt.custom_sales_order as sales_order,
		       coalesce(
		           (select group_concat(distinct dti.item_code order by dti.idx separator ', ')
		            from `tabDelivery Trip Item` dti
		            where dti.parent = dt.name and dti.parenttype = 'Delivery Trip'),
		           dt.custom_item
		       ) as item,
		       dt.custom_delivery_location as delivery_location,
...
def get_columns():
	return [
		...
		# Data, not Link: a trip can carry several items, so this is a list.
		{"label": _("Items"), "fieldname": "item", "fieldtype": "Data", "width": 160},
		{"label": _("Delivery Location"), "fieldname": "delivery_location", "fieldtype": "Data", "width": 140},
