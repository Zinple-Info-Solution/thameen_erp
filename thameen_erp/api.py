...
def make_consolidated_invoice(customer, from_date=None, to_date=None, billing_month=None, company=None):
	...
	for note in notes:
		...
		if freight and note.custom_vehicle:
			key = (note.custom_vehicle, note.get("custom_transportation_item"))
			freight_by_vehicle.setdefault(key, 0)
			freight_by_vehicle[key] += freight
...
def _append_freight_lines(si, freight_by_vehicle, settings):
	"""One freight row per (vehicle, charge item) pair.

	Keyed on the item as well as the vehicle so a truck that ran under two
	different charge items in the same month produces two rows, each to its own
	revenue account, both against that vehicle's cost center.
	"""
	...
	for (vehicle, item_code), amount in freight_by_vehicle.items():
		item_code = item_code or settings.transportation_item
		if not item_code:
			missing.add(vehicle)
			continue
		...
		si.append(
			"items",
			{
				...
				"qty": 1,
				"rate": flt(amount),
				"income_account": _freight_income_account(item_code, si.company, settings),
	...
	if missing:
		frappe.msgprint(
			_(
				"Freight for {0} was skipped \u2014 no Transportation Charge Item on the trip or in Thameen Fleet Settings."
			).format(", ".join(sorted(missing))),
			indicator="orange",
		)


def _freight_income_account(item_code, company, settings):
	"""The item's own Item Default wins, so a dedicated freight item can carry
	its own revenue account. Falls back to the single account in Settings."""
	return (
		frappe.db.get_value(
			"Item Default",
		...
		or settings.transportation_income_account
	)
