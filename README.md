...
## 16. Truck stock, shortfalls and direct supply (v1.2)

Three numbers describe a truck and they are not interchangeable:

| Figure | Meaning | Where it comes from |
|---|---|---|
| Capacity | rated — what it carries when empty | typed on the Vehicle |
| Committed Qty | promised — planned qty on submitted open trips | recounted from trips (`vehicle_load.py`) |
| On Truck Qty | physical — actual stock in the vehicle warehouse | stored on the Vehicle, refreshed by a Stock Ledger Entry hook on every movement |
| Available Qty | room left — Capacity − max(Committed, On Truck) | recounted with the two above |

### Planning trips — two dialogs, one widget (`public/js/trip_planner.js`)
* **Sales Order → Create → Delivery Trips**: one row per site+item; *Plan by
  truck* splits by capacity into dated trips; every qty / truck / date editable;
  `make_delivery_trips_from_plan` cuts the pending SO rows to match.
* **Purchase Order → Create → Delivery Trips** (`po_trips.py`): asks
  **Supplier → My Warehouse** (inbound: PR onto the truck at Loading, Material
  Transfer truck → yard at Delivered, no DN, no POD) or **Supplier → Customer**
  (direct: PR onto the truck, DN from the truck against the chosen Sales Order).
  Never plans more than the order has pending (ordered − received − on trips).
* **Delivery Trip split dialog**: the plan table is editable the same way;
  `split_trip(plan=…)` honours it exactly via `allocate_to_plan`.
New trip fields: `custom_destination_type` (Customer / Own Warehouse),
`custom_target_warehouse`. Stock Entry load type `Inbound Unload`.

### Connections
`dashboards.py` adds Delivery Trip to the Sales Order and Purchase Order
connections panel, and PO / PR / Stock Entry / DN / MR to the trip's.

### Manual load / unload (Vehicle → Stock)
`Stock > Load Stock` opens a dialog: pick a warehouse, add items, see what is on
the truck now and what it will hold afterwards. Over capacity is a confirmed
warning (or a refusal — `Allow Manual Loading Above Capacity` in settings).
Insufficient stock is always refused, with a `Create Purchase Order` button for
the shortfall. `Unload Stock` is the reverse and pre-fills with what is on the
truck. Both are Material Transfers tagged `custom_vehicle_load_type`.

### Trip loading reuses what is already on the truck
`load_vehicle` asks `free_truck_stock()` — stock on the truck minus what other
trips at Loading/In Transit already planned — and transfers only the shortfall.
A truck loaded by hand in the morning is not loaded again in the afternoon.

### Stock check and procurement (Delivery Trip → Check Stock)
`procurement.check_trip_stock` reports per row: on truck (free), in loading
warehouse, short. When short, a dialog offers `Purchase Order for Shortfall`
(into the loading warehouse), `Material Request`, or `Switch to Direct Supply`.
The PO is always created in Draft: dispatch plans, purchasing commits money.

### Direct from Supplier
`Supply Source = Direct from Supplier` + Supplier + `Create > Purchase Order`.
At **Loading** a Purchase Receipt books the cement supplier → vehicle warehouse
(`receive_onto_vehicle`). At **Delivered** the Delivery Note is raised from the
vehicle warehouse exactly as before, so month-end billing and Vehicle
Profitability need no change. ERPNext's native drop-ship is deliberately not
used: it skips the Delivery Note, and the Delivery Note is what this app bills
from. Trip rows keep `purchase_order / po_detail / purchase_receipt /
purchase_receipt_item` for plant → truck → site traceability.

### One item per trip
`One Item per Trip` (settings, default on): the planner creates one trip per
site *and* item; a mixed draft is **split automatically on save** (one trip per
item, `after_save` → `split_trip_by_item`), refused at submit as a safety net,
and `Split by Item` remains as a manual fallback; the capacity split never mixes items. Turn it off to get the
pre-v1.2 behaviour (Part 8 of the original UAT script assumes it is off).
